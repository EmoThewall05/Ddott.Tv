#!/usr/bin/env python3
"""
ddott_shorts_like_fix_v2.py
Updates bindShortLike() so that:
 - Normal like (short tap): NO coins, just toggles the heart to liked (one-way)
 - Super like (long press 600ms+): +5 coins only, then fully locked

Usage:
    python3 ddott_shorts_like_fix_v2.py player/player-v3.html
"""
...
NEW_FN = (
    ...
    "state.superLiked=true;state.liked=true;likeCount+=5;coins+=5;"
    ...
    "updateCoins();showCoinBurst(btn);showToast('Super Like! +5 🪙');"
...
def fix(path):
    ...
    print("✅ bindShortLike() updated — like gives 0 coins, super like gives +5 only. Saved to:", path)


if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 ddott_shorts_like_fix_v2.py path/to/player-v3.html")
        sys.exit(1)
    fix(sys.argv[1])
